Contributing to Pitcore
=======

We're working hard to make *pitcore* the most powerful JavaScript library for working with pitcoin. Our goal is to have *pitcore* be a library that can be used by anyone interested in pitcoin, and to level expertise differences with great design and documentation.
...
These are some global design goals in pitcore that any change must adhere.
...
When possible, pitcore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Pitcore methods. For example, calling `JSON.stringify` on a Pitcore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'pitcore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the pitcore object.

DO:
```javascript
var pitcore = require('../');
var PublicKey = pitcore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add alkaperl git@github.com:alkaperl/pitcore.git
git pull --rebase alkaperl master
```
...
```
Finally go to [github.com/alkaperl/pitcore](https://github.com/alkaperl/pitcore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of pitcore.

If you have any questions feel free to post them to
[github.com/alkaperl/pitcore/issues](https://github.com/alkaperl/pitcore/issues).
